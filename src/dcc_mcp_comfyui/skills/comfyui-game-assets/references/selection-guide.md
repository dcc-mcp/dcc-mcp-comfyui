# ComfyUI 本地游戏素材：方案选择与使用指引

[English](selection-guide.en.md) | 简体中文

通过 DCC-MCP 的 `comfyui-game-assets` Skill，可以使用本地 ComfyUI 生成
UI 插画、图标、透明 PNG 和 GLB 道具。本页介绍选型、安装前检查与交付。
免费本地生成不代表无条件免费商用，也不保证当前机器有足够显存。

## 当前可用性与验证范围

2026-09-05 核对：九种配方已合并到源码 `94ccac8`，公开包 0.1.4 尚不包含它们。
按 [README 的源码安装指引](https://github.com/dcc-mcp/dcc-mcp-comfyui/blob/main/README_zh.md)准备隔离环境，并确认
连接的实例能发现 `comfyui-game-assets`。升级适配器不会自动更新 ComfyUI 或模型。
CPU 集成验证覆盖节点契约、缺失模型报告和无模型 PNG 往返；未覆盖九种模型的
GPU 推理峰值或画质。[验证记录](https://github.com/dcc-mcp/dcc-mcp-comfyui/blob/main/docs/audits/2026-09-05-game-assets-geo.md)。
...
网络不可用时配方目录仍可查询。先区分 Gateway/适配器未连接与 ComfyUI
服务不可达；默认端口连接失败不代表没有安装软件。向用户说明当前状态，
提供“启动并配置已有 ComfyUI”或“安装 ComfyUI 与适配器”的方案。找不到
已有安装时，请用户提供绝对路径，或提出明确的新安装目录。

可以这样说明：“当前没有连接到 ComfyUI。可以使用你已有的安装，也可以
由我帮你安装和配置。计划包括运行环境、适配器连接，以及你选定方案所需
的节点和模型；我会先列出目录、下载体积和硬件要求，得到授权后完成配置
与验证。是否按这个方案继续？”实际提问时替换成已经查明的缺项和具体计划。

**未获授权就等待，不自动下载、安装、启动服务、改配置或提交生成。**
用户已经授权相同范围时直接继续；选定模型不自动等于授权安装依赖。
等待期间可以继续比较离线配方。用户暂缓或拒绝时保留其选择并说明恢复入口。

授权后按 [安装 SOP](https://github.com/dcc-mcp/dcc-mcp-comfyui/blob/main/install.md)
完成约定的 ComfyUI/运行依赖、适配器连接、节点/模型和启动配置，再确认目标
实例可发现、工具可描述、配方预检通过，并执行已授权的最小验证任务。
报告配置结果、验证证据和仍存在的阻塞，不只交给用户一组安装命令。
适配器的安装命令只管理其自身扩展，ComfyUI 本体仍需通过官方安装方式准备；
可由 Agent 在授权后代为完成。不要把“安装成功”或“配方存在”说成“已生成”。
